...
import { Target, Trash2, Gauge, RotateCcw, Settings, Sliders, Layers, Eye, EyeOff, Plus } from 'lucide-react';
...
export function PitchTracker({
  ...
  const speeds = pitches.map(p => p.velocity);
  ...
  const getPitchColor = (type: PitchType) => {
    switch (type) {
  ...
  const getPitchHexColor = (type: PitchType) => {
    switch (type) {
  ...
  return (
    <div className="flex flex-col gap-5 h-full overflow-y-auto pr-1">
      {/* Pitch Selector & Calibration Panel */}
      ...
      <div className="grid grid-cols-3 gap-2">
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
          <p className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">Pitches</p>
          <p className="text-2xl font-mono text-white font-bold leading-none mt-1.5">{totalPitches}</p>
          <p className="text-[9px] text-slate-400 mt-1">{strikes} Str - {totalPitches - strikes} Bal</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
          <p className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">Strike %</p>
          <p className={`text-2xl font-mono font-bold leading-none mt-1.5 ${strikePercentage >= 60 ? 'text-emerald-400' : strikePercentage >= 45 ? 'text-amber-400' : 'text-slate-400'}`}>
            {strikePercentage}%
          </p>
          <p className="text-[9px] text-slate-400 mt-1">Zone Accuracy</p>
        </div>
        <div className="bg-slate-900 border border-slate-800 rounded-xl p-3 text-center">
          <p className="text-[9px] text-slate-500 uppercase font-bold tracking-wider">Max Velo</p>
          <p className="text-2xl font-mono text-sky-400 font-bold leading-none mt-1.5">{maxSpeed ? `${maxSpeed}` : '--'}</p>
          <p className="text-[9px] text-slate-400 mt-1">Avg: {avgSpeed ? `${avgSpeed} mph` : '--'}</p>
        </div>
      </div>

      {/* 2D Mini Strike Zone Map & Legend */}
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-xl p-4 flex flex-col flex-1 min-h-[220px]">
        ...
        {/* Logs Table */}
        <div className="flex-1 overflow-y-auto max-h-[240px] pr-0.5 border border-slate-800 bg-slate-950/20 rounded-lg">
          {pitches.length === 0 ? (
            <div className="h-full flex flex-col items-center justify-center p-6 text-center text-slate-500">
              ...
              <p className="text-[10px] text-slate-600 mt-1 max-w-[180px]">
                Click on the main video feed or the Interactive PitchCast map above to log a pitch.
              </p>
            </div>
          ) : (
